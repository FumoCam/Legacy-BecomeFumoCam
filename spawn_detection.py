...
def match_and_draw(img1_obj, img2_obj):
    ...
    vis = explore_match("Spawnpoint Match", img1_obj["img"], img2_obj["img"], kp_pairs, status, H)
...
def main():
    ...
    for spawn in spawns:
        ...
        for lighting_cycle in ["day","night"]:
            features_path = os.path.join("resources", "spawns", spawn, lighting_cycle)
            confidence_scores = []
            for file_name in os.listdir(features_path):
                ...
                img2_obj["kp"], img2_obj["desc"] = detector.detectAndCompute(img2_obj["img"], None)
